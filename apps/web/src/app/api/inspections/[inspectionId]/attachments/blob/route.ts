  ...
  try {
    ...
    const response = await handleUpload({
      ...
      onBeforeGenerateToken: async (pathname) => {
        if (!pathname.startsWith(`${tenantPrefix}/uploaded-pdf/${sanitizePathSegment(inspectionId)}-`)) {
          throw new Error("Upload path is invalid for this inspection.");
        }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to prepare PDF upload." },
      { status: 400 }
    );
  }
}
