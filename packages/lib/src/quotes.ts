...
export const quoteInputSchema = z.object({
  ...
  includeDepositRequirement: z.boolean().default(false),
...
export async function createQuote(actor: ActorContext, input: QuoteInput) {
  ...
  const quote = await prisma.quote.create({
    data: {
      ...
      includeDepositRequirement: parsedInput.includeDepositRequirement,
...
export async function getAuthorizedQuotePdf(actor: ActorContext, quoteId: string) {
  ...
  const pdfBytes = await generateQuotePdf({
    ...
    quote: {
      ...
      includeDepositRequirement: quote.includeDepositRequirement,
