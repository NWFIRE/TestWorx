...
export function allowsMultipleInspectionTasks(inspectionType: keyof typeof inspectionTypeRegistry) {
  return multiSystemInspectionTypeSet.has(inspectionType as InspectionType);
}
...
type ServiceScheduleInspectionPlanTask = ScheduledInspectionTaskInput & {
  sourceTaskIndex: number;
  duePeriod: string;
  isGeneratedFutureOccurrence: boolean;
};

type ServiceScheduleInspectionPlanGroup = {
  duePeriod: string;
  scheduledStart: Date;
  isCurrentPeriod: boolean;
  tasks: ServiceScheduleInspectionPlanTask[];
};

function getTaskDuePeriod(input: {
  task: Pick<ScheduledInspectionTaskInput, "dueMonth" | "dueDate">;
  fallbackScheduledStart: Date;
}) {
  return normalizeTaskDueMonth({
    dueMonth: input.task.dueMonth,
    dueDate: input.task.dueDate ?? null,
    fallbackMonth: format(input.fallbackScheduledStart, "yyyy-MM")
  }) ?? format(input.fallbackScheduledStart, "yyyy-MM");
}

function getScheduledStartForDuePeriod(input: {
  duePeriod: string;
  baseScheduledStart: Date;
  dueDate?: Date | null;
}) {
  if (
    input.dueDate instanceof Date &&
    !Number.isNaN(input.dueDate.getTime()) &&
    !isPlaceholderMonthAnchorDueDate({ dueDate: input.dueDate, dueMonth: input.duePeriod })
  ) {
    return input.dueDate;
  }

  const [yearText, monthText] = input.duePeriod.split("-");
  const year = Number(yearText);
  const month = Number(monthText);
  if (!Number.isFinite(year) || !Number.isFinite(month) || month < 1 || month > 12) {
    return input.baseScheduledStart;
  }

  const scheduledStart = new Date(input.baseScheduledStart);
  scheduledStart.setFullYear(year, month - 1, 1);
  return scheduledStart;
}

export function buildServiceScheduleInspectionPlan(input: {
  tasks: ScheduledInspectionTaskInput[];
  scheduledStart: Date;
  inspectionMonth?: string | null;
}): ServiceScheduleInspectionPlanGroup[] {
  const currentPeriod = input.inspectionMonth ?? format(input.scheduledStart, "yyyy-MM");
  const taskOccurrences: ServiceScheduleInspectionPlanTask[] = [];

  input.tasks.forEach((task, sourceTaskIndex) => {
    const duePeriod = getTaskDuePeriod({ task, fallbackScheduledStart: input.scheduledStart });
    taskOccurrences.push({
      ...task,
      assignedTechnicianId: duePeriod === currentPeriod ? task.assignedTechnicianId ?? null : null,
      dueMonth: duePeriod,
      schedulingStatus: "scheduled_now",
      sourceTaskIndex,
      duePeriod,
      isGeneratedFutureOccurrence: false
    });

    const dueAnchor = getScheduledStartForDuePeriod({
      duePeriod,
      baseScheduledStart: input.scheduledStart,
      dueDate: task.dueDate ?? null
    });
    const nextDueDate = nextDueFrom(dueAnchor, task.frequency);
    if (nextDueDate && duePeriod === currentPeriod) {
      const nextDuePeriod = format(nextDueDate, "yyyy-MM");
      taskOccurrences.push({
        ...task,
        assignedTechnicianId: null,
        dueMonth: nextDuePeriod,
        dueDate: nextDueDate,
        schedulingStatus: "scheduled_now",
        recurrenceAnchorScheduledStart: dueAnchor,
        recurrenceNextDueAt: nextDueFrom(nextDueDate, task.frequency),
        sourceTaskIndex,
        duePeriod: nextDuePeriod,
        isGeneratedFutureOccurrence: true
      });
    }
  });
  ...
  for (const task of dedupedOccurrences) {
    ...
    grouped.set(task.duePeriod, {
      duePeriod: task.duePeriod,
      scheduledStart: getScheduledStartForDuePeriod({
        duePeriod: task.duePeriod,
        baseScheduledStart: input.scheduledStart,
        dueDate: task.dueDate ?? null
      }),
      isCurrentPeriod: task.duePeriod === currentPeriod,
      tasks: [task]
    });
  }

  return [...grouped.values()].sort((left, right) => {
    if (left.isCurrentPeriod !== right.isCurrentPeriod) {
      return left.isCurrentPeriod ? -1 : 1;
    }
    return left.scheduledStart.getTime() - right.scheduledStart.getTime();
  });
}

type TechnicianEligibilityMode = "assign" | "claim";

type TechnicianEligibilityRow = {
...
export function isTechnicianEligibleForReportTypesFromRows(input: {
  rows: TechnicianEligibilityRow[];
...
function getCurrentVisitReportTypes(tasks: Array<{ inspectionType: keyof typeof inspectionTypeRegistry; schedulingStatus?: string | null }>) {
  return tasks
    .filter((task) => isCurrentVisitTaskSchedulingStatus(task.schedulingStatus ?? "scheduled_now"))
...
async function validateSchedulingReferences(tx: Prisma.TransactionClient, tenantId: string, input: z.infer<typeof scheduleInspectionSchema>) {
  const uniqueAssignedTechnicianIds = normalizeAssignedTechnicianIds({
    assignedTechnicianIds: [
      ...(input.assignedTechnicianIds ?? []),
      ...input.tasks
        .filter((task) => isCurrentVisitTaskSchedulingStatus(task.schedulingStatus))
        .map((task) => task.assignedTechnicianId)
  ...
  const currentVisitReportTypes = getCurrentVisitReportTypes(input.tasks);
  if (currentVisitReportTypes.length > 0) {
    await Promise.all(
      assignedTechnicianIds.map((technicianUserId) =>
        assertTechnicianEligibilityForReportTypes({
          tx,
          tenantId,
          technicianUserId,
          reportTypes: currentVisitReportTypes,
          mode: "assign"
        })
      )
    );
...
async function createServiceSchedulesForInspectionInput(input: {
  tx: Prisma.TransactionClient;
  tenantId: string;
  customerCompanyId: string;
  siteId: string;
  ...
  const scheduleIdsByTaskIndex = new Map<number, string>();
  await Promise.all(
    input.tasks.map(async (task, index) => {
      const dueMonth = getTaskDuePeriod({ task, fallbackScheduledStart: input.scheduledStart });
      const dueDate = getScheduledStartForDuePeriod({
        duePeriod: dueMonth,
        baseScheduledStart: input.scheduledStart,
        dueDate: task.dueDate ?? null
      });
      const serviceSchedule = await input.tx.serviceSchedule.create({
        data: {
          ...
          nextDueDate: nextDueFrom(dueDate, task.frequency),
          dueMonth,
          dueDayOrWindow: task.dueDate && !isPlaceholderMonthAnchorDueDate({ dueDate: task.dueDate, dueMonth })
            ? format(task.dueDate, "yyyy-MM-dd")
            : null,
          isActive: true
        }
      });
      scheduleIdsByTaskIndex.set(index, serviceSchedule.id);
    })
  );
...
async function updateStartedInspectionTasksInPlace(input: {
  tx: Prisma.TransactionClient;
  tenantId: string;
  inspectionId: string;
  scheduledStart: Date;
  ...
  nextTasks: ScheduledInspectionTaskInput[];
}) {
  const taskQueues = new Map<keyof typeof inspectionTypeRegistry, typeof input.existingTasks>();
  for (const task of input.existingTasks) {
    const queue = taskQueues.get(task.inspectionType) ?? [];
  ...
  for (const [index, task] of input.nextTasks.entries()) {
    const queue = taskQueues.get(task.inspectionType) ?? [];
    const existingTask = queue.shift();
    taskQueues.set(task.inspectionType, queue);
    ...
    if (existingTask) {
      await input.tx.inspectionTask.update({
        ...
        data: {
          ...
          dueMonth: normalizeTaskDueMonth({
            dueMonth: task.dueMonth,
            dueDate: task.dueDate ?? null,
            fallbackMonth: format(input.scheduledStart, "yyyy-MM")
          }),
          dueDate: task.dueDate ?? null,
          schedulingStatus: task.schedulingStatus ?? "scheduled_now",
          notes: task.notes,
      ...
      if (existingTask.recurrence) {
        const anchorScheduledStart = task.recurrenceAnchorScheduledStart ?? existingTask.recurrence.anchorScheduledStart ?? input.scheduledStart;
        await input.tx.inspectionRecurrence.update({
          where: { id: existingTask.recurrence.id },
          data: {
            frequency: task.frequency,
            anchorScheduledStart,
            nextDueAt: task.recurrenceNextDueAt ?? nextDueFrom(anchorScheduledStart, task.frequency)
          }
        });
      } else {
        await input.tx.inspectionRecurrence.create({
          data: {
            tenantId: input.tenantId,
            inspectionTaskId: existingTask.id,
            seriesId: task.recurrenceSeriesId ?? crypto.randomUUID(),
            frequency: task.frequency,
            anchorScheduledStart: task.recurrenceAnchorScheduledStart ?? input.scheduledStart,
            nextDueAt: task.recurrenceNextDueAt ?? nextDueFrom(task.recurrenceAnchorScheduledStart ?? input.scheduledStart, task.frequency)
          }
        });
      }
...
async function createRecurringFollowUpInspectionsTx(tx: Prisma.TransactionClient, input: {
  ...
  for (const group of [...groupedTasks.values()].sort((left, right) => left.scheduledStart.getTime() - right.scheduledStart.getTime())) {
    ...
    await syncInspectionTechnicianAssignments(tx, createdInspection.id, input.tenantId, []);
...
export async function createInspection(actor: ActorContext, input: z.infer<typeof scheduleInspectionSchema>) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const serviceScheduleIdsByTaskIndex = await createServiceSchedulesForInspectionInput({
      tx,
      tenantId,
      customerCompanyId: customerCompany.id,
      siteId: site.id,
    ...
    const plannedInspectionGroups = buildServiceScheduleInspectionPlan({
      tasks: input.tasks.map((task, index) => ({
        ...task,
        serviceScheduleId: serviceScheduleIdsByTaskIndex.get(index) ?? null
      })),
      scheduledStart: input.scheduledStart,
      inspectionMonth: input.inspectionMonth
    });
    if (!plannedInspectionGroups.length) {
      throw new Error("Select at least one due service before creating the inspection.");
    }

    const createdInspections: Array<{ id: string; isCurrentPeriod: boolean }> = [];
    for (const group of plannedInspectionGroups) {
      const groupAssignedTechnicianIds = group.isCurrentPeriod ? assignedTechnicianIds : [];
      const groupPrimaryTechnicianId = group.isCurrentPeriod ? primaryAssignedTechnicianId : null;
      const groupPriorityState = group.isCurrentPeriod
        ? priorityState
        : { isPriority: false, priorityAssignedAt: null, priorityClearedAt: null };
      const inspection = await tx.inspection.create({
        data: {
          tenantId,
          customerCompanyId: customerCompany.id,
          siteId: site.id,
          assignedTechnicianId: groupPrimaryTechnicianId,
          createdByUserId: parsedActor.userId,
          inspectionClassification: input.inspectionClassification,
          isPriority: groupPriorityState.isPriority,
          priorityAssignedAt: groupPriorityState.priorityAssignedAt,
          priorityClearedAt: groupPriorityState.priorityClearedAt,
          scheduledStart: group.scheduledStart,
          scheduledEnd: group.isCurrentPeriod ? input.scheduledEnd ?? null : null,
          status: group.isCurrentPeriod ? input.status : InspectionStatus.to_be_completed,
          notes: input.notes,
          claimable: groupAssignedTechnicianIds.length === 0
        }
      });
      createdInspections.push({ id: inspection.id, isCurrentPeriod: group.isCurrentPeriod });
      ...
      await syncInspectionTechnicianAssignments(tx, inspection.id, tenantId, groupAssignedTechnicianIds);
      await writeInspectionTasks(
        tx,
        inspection.id,
        tenantId,
        ...
        group.tasks.map((task) => ({
          ...task,
          assignedTechnicianId: group.isCurrentPeriod ? task.assignedTechnicianId ?? groupPrimaryTechnicianId : null
        }))
      );
      await createAuditLog(tx, {
        tenantId,
        actorUserId: parsedActor.userId,
        action: group.isCurrentPeriod ? "inspection.created" : "inspection.future_created_from_service_schedule",
        ...
        metadata: {
          ...
          isPriority: groupPriorityState.isPriority,
          status: group.isCurrentPeriod ? input.status : InspectionStatus.to_be_completed,
          duePeriod: group.duePeriod,
          taskCount: group.tasks.length,
          sourceServiceTaskIndexes: [...new Set(group.tasks.map((task) => task.sourceTaskIndex))]
        }
      });
      ...
      if (groupPriorityState.isPriority) {
        await createAuditLog(tx, {
          tenantId,
          actorUserId: parsedActor.userId,
      ...
      if (groupAssignedTechnicianIds.length > 0) {
        await createAuditLog(tx, {
          tenantId,
          actorUserId: parsedActor.userId,
          ...
          entityId: inspection.id,
          metadata: {
            assignedTechnicianIds: groupAssignedTechnicianIds
          }
        });
      }

      if (groupPriorityState.isPriority && groupAssignedTechnicianIds.length > 0) {
        await createPriorityInspectionAssignedNotificationsTx(tx, {
          tenantId,
          inspectionId: inspection.id,
          technicianIds: groupAssignedTechnicianIds
        });
      }
    }
...
export async function updateInspection(actor: ActorContext, inspectionId: string, input: z.infer<typeof scheduleInspectionSchema>) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (reportActivityCount > 0) {
      ...
      await updateStartedInspectionTasksInPlace({
        tx,
        tenantId,
        inspectionId,
        scheduledStart: input.scheduledStart,
        existingTasks: existing.tasks,
        nextTasks: taskDefinitions
      });
      ...
      await createAuditLog(tx, {
        ...
        entityId: inspectionId,
        metadata: {
          editedStartedVisit: true,
          reportActivityCount,
          preservedExistingReports: true,
...
async function ensureServiceSchedulesForCompletedRecurringTasks(input: {
  ...
  const completedRecurringTasks = await prisma.inspectionTask.findMany({
    where: {
      tenantId: input.tenantId,
...
async function generateMissingInspectionsFromServiceSchedules(input: {
  tenantId: string;
  actorUserId: string;
  rangeStart: Date;
  rangeEnd: Date;
  ...
  let advancedScheduleCount = 0;
  for (const schedule of dueSchedules) {
    if (advancedScheduleCount >= upcomingServiceScheduleGenerationLimits.serviceSchedulesAdvancedPerRequest) {
      break;
    }
...
async function ensureUpcomingServiceScheduleInspections(input: {
  ...
  if (!await hasServiceScheduleInfrastructure(prisma)) {
    return {
  ...
  const serviceSchedulesBackfilled = await ensureServiceSchedulesForCompletedRecurringTasks(input);
  const inspectionsGenerated = await generateMissingInspectionsFromServiceSchedules(input);
  ...
  const searchFilter = requestedQuery
    ? {
        OR: [
          ...
          { customerCompany: { billingEmail: { contains: requestedQuery, mode: "insensitive" as const } } },
          { customerCompany: { phone: { contains: requestedQuery, mode: "insensitive" as const } } },
          { customerCompany: { serviceAddressLine1: { contains: requestedQuery, mode: "insensitive" as const } } },
          { customerCompany: { serviceAddressLine2: { contains: requestedQuery, mode: "insensitive" as const } } },
          { customerCompany: { serviceCity: { contains: requestedQuery, mode: "insensitive" as const } } },
          { customerCompany: { serviceState: { contains: requestedQuery, mode: "insensitive" as const } } },
          { customerCompany: { servicePostalCode: { contains: requestedQuery, mode: "insensitive" as const } } },
          { customerCompany: { billingAddressLine1: { contains: requestedQuery, mode: "insensitive" as const } } },
          { customerCompany: { billingAddressLine2: { contains: requestedQuery, mode: "insensitive" as const } } },
          { customerCompany: { billingCity: { contains: requestedQuery, mode: "insensitive" as const } } },
          { customerCompany: { billingState: { contains: requestedQuery, mode: "insensitive" as const } } },
          { customerCompany: { billingPostalCode: { contains: requestedQuery, mode: "insensitive" as const } } },
  ...
  const queryScope: Prisma.InspectionWhereInput | null = requestedQuery
    ? {
        OR: [
          ...
          { customerCompany: { contactName: { contains: requestedQuery, mode: "insensitive" } } },
          { customerCompany: { contactEmails: { contains: requestedQuery, mode: "insensitive" } } },
          { customerCompany: { billingEmail: { contains: requestedQuery, mode: "insensitive" } } },
          { customerCompany: { phone: { contains: requestedQuery, mode: "insensitive" } } },
          { customerCompany: { serviceAddressLine1: { contains: requestedQuery, mode: "insensitive" } } },
          { customerCompany: { serviceAddressLine2: { contains: requestedQuery, mode: "insensitive" } } },
          { customerCompany: { serviceCity: { contains: requestedQuery, mode: "insensitive" } } },
          { customerCompany: { serviceState: { contains: requestedQuery, mode: "insensitive" } } },
          { customerCompany: { servicePostalCode: { contains: requestedQuery, mode: "insensitive" } } },
          { customerCompany: { billingAddressLine1: { contains: requestedQuery, mode: "insensitive" } } },
          { customerCompany: { billingAddressLine2: { contains: requestedQuery, mode: "insensitive" } } },
          { customerCompany: { billingCity: { contains: requestedQuery, mode: "insensitive" } } },
          { customerCompany: { billingState: { contains: requestedQuery, mode: "insensitive" } } },
          { customerCompany: { billingPostalCode: { contains: requestedQuery, mode: "insensitive" } } },
...
export async function claimInspection(actor: ActorContext, inspectionId: string) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await assertTechnicianEligibilityForReportTypes({
      tx,
      tenantId,
