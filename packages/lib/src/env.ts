...
function parseServerEnv(raw: NodeJS.ProcessEnv) {
  const parsed = baseServerEnvSchema.safeParse({
    ...
    AUTH_SECRET: raw.AUTH_SECRET?.trim(),
    NEXTAUTH_URL: raw.NEXTAUTH_URL?.trim(),
    APP_URL: raw.APP_URL?.trim(),
    STORAGE_DRIVER: raw.STORAGE_DRIVER?.trim() ?? "inline",
    BLOB_READ_WRITE_TOKEN: raw.BLOB_READ_WRITE_TOKEN?.trim(),
    STRIPE_PUBLISHABLE_KEY: raw.STRIPE_PUBLISHABLE_KEY?.trim(),
    STRIPE_SECRET_KEY: raw.STRIPE_SECRET_KEY?.trim(),
    STRIPE_WEBHOOK_SECRET: raw.STRIPE_WEBHOOK_SECRET?.trim(),
    STRIPE_PRICE_STARTER: raw.STRIPE_PRICE_STARTER?.trim(),
    STRIPE_PRICE_PROFESSIONAL: raw.STRIPE_PRICE_PROFESSIONAL?.trim(),
    STRIPE_PRICE_ENTERPRISE: raw.STRIPE_PRICE_ENTERPRISE?.trim(),
    QUICKBOOKS_CLIENT_ID: raw.QUICKBOOKS_CLIENT_ID?.trim(),
    QUICKBOOKS_CLIENT_SECRET: raw.QUICKBOOKS_CLIENT_SECRET?.trim(),
    QUICKBOOKS_SANDBOX: raw.QUICKBOOKS_SANDBOX?.trim(),
    RESEND_API_KEY: raw.RESEND_API_KEY?.trim(),
    RESEND_FROM_EMAIL: raw.RESEND_FROM_EMAIL?.trim()
  });
