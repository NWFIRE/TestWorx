...
describe("quickbooks billing sync hardening", () => {
  ...
  it("uses linked catalog pricing when a persisted billing item is missing unit price", async () => {
    ...
    prismaMock.quickBooksCatalogItem.findFirst
      ...
      .mockResolvedValueOnce({
        quickbooksItemId: "mapped_item_1",
        name: "Recharge - 5# ABC",
        active: true,
        taxable: false
      });
      ...
      items: [
        {
          ...
          unit: "hours",
          category: "labor",
          linkedCatalogItemId: "catalog_fire_alarm_annual",
          linkedQuickBooksItemId: "qb_fire_alarm_annual",
          linkedCatalogItemName: "Fire Alarm - Annual Inspection"
        }
      ]
    });
    prismaMock.inspectionBillingSummary.update.mockResolvedValue(undefined);
  ...
  it("uses an embedded billing review catalog match before legacy ON_SITE_LABOR mapping", async () => {
    prismaMock.tenant.findUnique.mockResolvedValue(buildTenantConnection());
    prismaMock.customerCompany.findUnique.mockResolvedValue({ quickbooksCustomerId: null });
    prismaMock.customerCompany.update.mockResolvedValue(undefined);
    prismaMock.site.findFirst.mockResolvedValue(null);
      ...
      items: [
        {
          ...
          code: "ON_SITE_LABOR",
          currentCatalogMatch: {
            catalogItemId: "catalog_fire_alarm_annual",
            quickbooksItemId: "qb_fire_alarm_annual",
            name: "Fire Alarm - Annual Inspection"
          }
        }
  ...
  it("uses alias-backed catalog matches for on-site labor before legacy ON_SITE_LABOR mapping", async () => {
    prismaMock.tenant.findUnique.mockResolvedValue(buildTenantConnection());
    prismaMock.customerCompany.findUnique.mockResolvedValue({ quickbooksCustomerId: null });
    prismaMock.customerCompany.update.mockResolvedValue(undefined);
    prismaMock.site.findFirst.mockResolvedValue(null);
  ...
  it("applies catalog taxability to inspection billing invoice lines", async () => {
    prismaMock.tenant.findUnique.mockResolvedValue(buildTenantConnection());
    prismaMock.customerCompany.findUnique.mockResolvedValue({ quickbooksCustomerId: null });
    prismaMock.customerCompany.update.mockResolvedValue(undefined);
    prismaMock.site.findFirst.mockResolvedValue(null);
  ...
  it("stores a clean retry message when QuickBooks returns a send SystemFault", async () => {
    ...
    const firstSendRequest = fetchMock.mock.calls[4]?.[1] as RequestInit | undefined;
    expect((firstSendRequest?.headers as Record<string, string>)["Content-Type"]).toBeUndefined();
    expect(firstSendRequest?.body).toBeUndefined();
    expect(prismaMock.inspectionBillingSummary.update).toHaveBeenCalledWith({
      where: { id: "summary_1" },
      data: expect.objectContaining({
        quickbooksSyncStatus: "synced",
  ...
  it("automatically retries a transient QuickBooks send SystemFault before marking the invoice sent", async () => {
    prismaMock.tenant.findUnique.mockResolvedValue(buildTenantConnection());
    prismaMock.customerCompany.findUnique.mockResolvedValue({ quickbooksCustomerId: null });
    prismaMock.customerCompany.update.mockResolvedValue(undefined);
    prismaMock.site.findFirst.mockResolvedValue(null);
    ...
    fetchMock
      ...
      .mockResolvedValueOnce(jsonResponse({ Invoice: { Id: "invoice_1", DocNumber: "TW2026-1000" } }))
      .mockResolvedValueOnce(jsonResponse(systemFault, { status: 500 }))
      .mockResolvedValueOnce(jsonResponse({ Invoice: { Id: "invoice_1" } }));
    ...
    expect(result.quickbooksSendError).toBeNull();
  ...
  it("reuses an active compliance fee mapping for a newly assessed compliance division", async () => {
    prismaMock.tenant.findUnique.mockResolvedValue(buildTenantConnection());
    prismaMock.customerCompany.findUnique.mockResolvedValue({ quickbooksCustomerId: "qbo_customer_1" });
    prismaMock.inspectionBillingSummary.findUnique.mockResolvedValue({
      ...buildBillingSummary(),
    ...
    prismaMock.inspectionBillingSummary.update.mockResolvedValue(undefined);
    prismaMock.quickBooksItemMap.findUnique.mockResolvedValue(null);
    prismaMock.quickBooksItemMap.findFirst.mockResolvedValue({
      id: "map_compliance_kitchen",
      tenantId: "tenant_1",
      integrationId: "realm_1",
      internalCode: "COMPLIANCE_REPORTING_FEE_KITCHEN_SUPPRESSION",
      internalName: "Compliance Reporting Fee",
